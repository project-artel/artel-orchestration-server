...
/**
 * 화면에 대한 프레임을 agent 로 내보내는 자리 (ARTEL-655 · ARTEL-668 · ARTEL-910).
 *
 ...
 *
 * 셋 다 **보낼 곳이 없으면 `false` 를 돌려주고 던지지 않는다.** `false` 가 오는 자리는 둘이다 —
...
interface QaScreenFramePort {
    ...
    /**
     * 방금 생긴 화면을 무엇이라 부를지 묻는다 (ARTEL-910).
     *
     * 제안과 같이 답이 있는 질문이라 `correlationId` 를 싣는다. `false` 를 받은 쪽은 그 화면을
     * 아직 안 물어본 것으로 두어야 다음 기회에 다시 물을 수 있다 — 나가지 못한 질문을 물어본
     * 것으로 치면 그 화면은 영영 이름을 못 갖는다.
     *
     * @param messageId 이 질문의 식별자. 답의 `correlationId` 가 이 값으로 돌아온다.
     * @param summary QA 타임라인에 뜨는 한 줄.
     */
    suspend fun sendScreenName(
        gameInstanceId: Long,
        messageId: String,
        summary: String,
        payload: JsonNode,
