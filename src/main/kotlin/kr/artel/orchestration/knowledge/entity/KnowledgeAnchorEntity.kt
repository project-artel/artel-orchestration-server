...
data class KnowledgeAnchorEntity(
     ...
     * `anchor` 를 달 수 있는 시점이면 언제나 안다 — 그래서 NOT NULL 이다.
     *
     * content map 과 대조하지 않는다. content map 이 없는 프로젝트도 씬 이름은 있고, 검증하면 그
     * 프로젝트에서 오는 `anchor` 가 전부 거절된다.
     */
    @Column("scene_name")
    val sceneName: String,

    /**
     * 판정된 화면(`content_map.screen.id`). null 이면 씬까지만 아는 `anchor` 다.
     *
