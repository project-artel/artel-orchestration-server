...
object ScenarioFlowPlan {
     ...
     *
     * 근거는 실측이다. 처음에는 12·3 이었다 — 모델이 42건을 5~9개 흐름으로 내고 있었고, 지나갈
     * 자리는 하나만 있어도 "게임을 해서 거기까지 가라"는 말이라 셋을 넘기면 검증이 아니라 플레이가
     * 된다고 봤다.
     *
     * **20·6 으로 올렸다**(ARTEL-687). 그 판단이 한 판의 길이만 보고 **매 판의 준비 부담**을 안
     * 쟀다. 흐름이 끊길 때마다 다음 판은 게임 중간에서 시작하고, 실행하는 사람은 거기까지 다시
     * 가야 한다. 케이스 83건으로 재 보니 열 흐름 중 여덟이 못 이어서가 아니라 이 상한에 걸려
     * 끊겼고, 20·6 에서는 판이 여섯으로 줄면서 **어긋남도 2건에서 1건으로 줄었다.**
     *
     * **정답이 아니라 출발점이고, 부르는 쪽이 바꿀 수 있다** — 지금은 `ScenarioRepairProperties`
     * 가 설정으로 들고 있고 거기에 셋을 나란히 잰 표가 있다.
     */
    const val MAX_CASES = 20
