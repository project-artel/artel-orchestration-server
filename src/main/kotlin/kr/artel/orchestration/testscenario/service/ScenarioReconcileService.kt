 ...
 *
 * **재설계**: 시나리오 본문 = **payload(JSONB)의 steps 리스트** 하나. 별도 케이스 조합 테이블
 * (test_scenario_case)은 폐기됐다 — 각 스텝은 행위 하나이며 검증 대상 TC를 `caseId`로 옵션 참조한다.
 * 따라서 upsert는 payload{title, description, steps} 통째 저장뿐이다.
 *
 * 항목별로 [ScenarioResult.scenarioId]로 분기한다:
 * - `null` → 새 시나리오 INSERT + 런 끝에 append.
 * - 값 있음 → 그 기존 시나리오 payload UPDATE(방어적으로 같은 프로젝트 소속일 때만).
 *
