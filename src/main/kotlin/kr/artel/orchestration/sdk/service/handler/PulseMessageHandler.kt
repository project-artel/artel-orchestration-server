 ...
 *
 * 전량 `pulse` 와 델타 `pulse` 를 가리지 않는다. 어느 쪽인지는 문서의 `whole` 이 말하고, 그 판단은
 * 받는 쪽 몫이다.
 */
@Component
class PulseMessageHandler(
    ...
    override suspend fun handle(instanceId: String, payloadText: String, session: WebSocketSession) {
        // 본문은 찍지 않는다. 전량 `pulse` 가 실측 약 18 KB 이고 초당 한 번 오므로, 다른 핸들러처럼
        // 원문을 로그에 실으면 `pulse` 만으로 로그가 채워진다. 크기는 남겨 둔다 — 적재량을 나중에
        // 되짚을 수 있는 값이 그것뿐이다.
        logger.info("pulse 수신 [instanceId: $instanceId]: ${payloadText.length}자")

        val gameInstanceId = instanceId.toLong()
        qaBridge.routePulse(gameInstanceId, payloadText)

        // 중계 **뒤에** 부른다. 앞에 두면 화면 적재가 느릴 때 `pulse` 가 agent 에 늦게 닿는다.
        //
