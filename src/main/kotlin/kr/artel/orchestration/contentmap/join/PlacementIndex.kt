...
class PlacementIndex private constructor(
    ...
    companion object {
        ...
        fun build(document: EvidenceDocumentModel): PlacementIndex {
            ...
            for (obj in document.allObjects) {
                ...
                for (type in obj.components.map { it.type }.distinct()) {
                    placementsByType.getOrPut(type) { mutableListOf() }.add(placement)
                }
            }
...
internal fun SceneObject.toPlacement(): ScenePlacement =
    ScenePlacement(scene = scene, path = path, selector = selector, label = label)
