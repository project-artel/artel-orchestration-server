...
/**
 * `DontDestroyOnLoad` 로 씬을 넘어 사는 오브젝트를 **실제로 동작하는 씬**에 귀속시킨다.
 *
 * 근거는 그런 오브젝트를 `persistentObjects` 에 따로 담고, 그 `scene` 칸에 씬이 아닌 이름
 * (`DontDestroyOnLoad`)을 적는다. 배치 색인이 그 이름을 씬으로 받아들이면 지도에 **아무도 갈 수 없는
 * 항목**이 생기고, 거기 앉은 기능으로 만든 테스트케이스는 사전조건이 "`DontDestroyOnLoad` 씬이 실행
 * 중이다" 가 되어 실행할 수 없다. 실측(`wv-editor-play-schema7.json`)에서 기능 469건 중 64건이
 * 그렇게 됐다.
 *
 ...
 * ## 무엇을 씬으로 치나
 *
 * 문서의 `scenes` 배열에 있는 이름만 씬이다. `DontDestroyOnLoad` 를 문자열로 지목하지 않는 이유는
 * 그것이 오늘 Unity 가 쓰는 이름일 뿐이기 때문이다 — 엔진이 다른 이름을 쓰거나 수집기가 또 다른
 * 가짜 씬을 만들어도 같은 규칙이 걸린다. `scenes` 가 비어 있는 문서에서는 이 판정을 하지 않는다.
 * 그때는 모든 오브젝트가 가짜 씬에 있다고 읽혀 지도가 통째로 비기 때문이다.
 *
 * ## 판단 단위는 record 가 아니라 **오브젝트 root**
 *
 * `DontDestroyOnLoad` 는 루트 오브젝트를 자식째 옮긴다. record 마다 따로 판정하면 같은 오브젝트의
 * `Start` 와 `Update` 가 서로 다른 씬에 실린다 — 한 오브젝트가 두 화면에 나뉘어 앉는 것은 그
 ...
 * |---|---|---|
 * | [PersistentSceneRule.ALSO_PLACED] | 같은 컴포넌트 타입이 진짜 씬 오브젝트에도 놓여 있다 | `exact` |
 * | [PersistentSceneRule.ACTIVE_SCENE_TEST] | 조건이 `SceneManager.GetActiveScene().name` 을 씬 이름과 맞댄다 | `exact` |
 * | [PersistentSceneRule.CONDITION_SUBJECT_PLACED] | 조건이 읽는 상태의 주인 타입이 **딱 한 씬에** 놓여 있다 | `derived` |
 *
 * 위 단계가 하나라도 씬을 내면 아래는 보지 않는다. 아래 단계가 더 넓은 답을 내기 때문이고, 넓은 답을
 * 좁은 답에 섞으면 확실한 자리에 불확실한 자리가 딸려 온다.
 *
 * 3단계가 **"딱 한 씬"을 요구하는 것이 이 규칙의 전부**다. 실측 `Tutorial.TutorialController` 는 조건에서
 * `MapMove.StagePosition` 과 `StoryController.IsAdvanceKeyDown()` 을 둘 다 읽는데, `Map.MapMove` 는
 * `Map_scene` 한 곳에만 놓였고 `Story.StoryController` 는 `StoryScene` 과 `EndingScene` 두 곳에 놓였다.
 * 자기 자리가 하나로 정해지지 않은 타입은 남의 자리도 정해 줄 수 없다 — 그래서 `MapMove` 만 anchor 가
 * 되고 튜토리얼은 `Map_scene` 하나로 앉는다. 이 조건이 없으면 튜토리얼이 스토리 씬까지 세 곳에
 * 복제되고, QA agent 는 없는 창을 찾으러 간다.
 *
 * ## 정하지 못하면 비운다
 *
 * anchor 가 하나도 없으면 씬 목록이 빈 채로 나가고, 그 root 의 기능은 **행이 되지 않는다.**
 * 아무 씬에나 붙이는 것이 여기서 할 수 있는 가장 나쁜 선택이라는 판단은 [EvidenceJoin] 이 씬 없는
 * record 에 대해 이미 내려 둔 것과 같다. 몇 건이 그렇게 빠졌는지는
...
class PersistentSceneAttribution(private val document: EvidenceDocumentModel) {
    ...
    /** 문서가 씬이라고 말한 이름들. 이 집합 밖의 `scene` 값은 씬이 아니다. */
    private val realSceneNames: Set<String> = document.scenes.toSet()

    /** 진짜 씬에 놓인 타입 → 그 타입이 놓인 씬들. 문서 순서. */
    ...
    /**
     * 이 오브젝트가 실제로 동작하는 자리들. 진짜 씬에 놓인 오브젝트는 자기 자신 하나를 그대로 낸다.
     *
     * 귀속하지 못한 persistent 오브젝트는 **빈 목록**이다 — 부르는 쪽이 그 오브젝트를 통째로 빼야
     * 한다는 뜻이고, 가짜 씬 이름이 색인에 들어가는 유일한 길이 여기서 막힌다.
     */
    ...
    /** 이 타입이 귀속 대상 persistent 오브젝트 위에만 사는가. 진짜 씬에도 놓였으면 false. */
    ...
    private fun attribute(rootPath: String, objects: List<SceneObject>): PersistentAttribution {
        ...
        val parkedAt = objects.first().let { "${it.scene}/$rootPath" }
        ...
        val anchors = PersistentSceneRule.entries
            .firstNotNullOfOrNull { rule -> anchorsBy(rule, parkedAt, types).distinct().takeIf { it.isNotEmpty() } }
            .orEmpty()
        return PersistentAttribution(rootPath, anchors)
    }

    private fun anchorsBy(rule: PersistentSceneRule, parkedAt: String, types: List<String>): List<SceneAnchor> =
        when (rule) {
            PersistentSceneRule.ALSO_PLACED -> alsoPlacedAnchors(parkedAt, types)
            PersistentSceneRule.ACTIVE_SCENE_TEST -> activeSceneAnchors(parkedAt, types)
            PersistentSceneRule.CONDITION_SUBJECT_PLACED -> conditionSubjectAnchors(parkedAt, types)
        }

    /**
     * 같은 컴포넌트 타입이 진짜 씬 오브젝트에도 놓여 있다.
     *
     * 실측 `Core.SaveLoadController` 와 `Combat.Stage.StageDataSingleton` 이 그렇다. 싱글턴이 씬마다
     * 자기 사본을 두고 첫 하나만 살아남는 흔한 모양이고, 문서가 "이 타입은 이 씬에 놓인다"고 직접
     * 말한 것이라 유도가 필요 없다.
     */
    private fun alsoPlacedAnchors(parkedAt: String, types: List<String>): List<SceneAnchor> =
        types.flatMap { type ->
            realScenesByType[type].orEmpty().map { scene ->
                SceneAnchor(
                    ...
                    steps = listOf(
                        AnchorStep(parkedAt, "component-type", type),
    ...
    /**
     * 조건이 활성 씬 이름을 씬 이름 리터럴과 맞댄다.
     *
     * `!=` 는 anchor 가 되지 않는다. "저 씬이 아니다"는 남은 씬이 여럿이라 자리를 하나로 좁히지
     * 못하고, 좁히지 못한 것을 anchor 라 부르면 나머지 씬 전부에 기능이 복제된다.
     */
    private fun activeSceneAnchors(parkedAt: String, types: List<String>): List<SceneAnchor> =
            ...
            record.condition.conjunctiveTests()
                ...
                .map { scene ->
                    SceneAnchor(
                        ...
                        steps = listOf(
                            AnchorStep(parkedAt, "condition-reads", ACTIVE_SCENE_NAME),
    ...
    /**
     * 조건이 읽는 상태의 주인 타입이 딱 한 씬에 놓여 있다.
     *
     * `MapMove.StagePosition <= 0` 의 주어는 `MapMove` 이고, `Map.MapMove` 는 `Map_scene` 에만 놓였다.
     * 그 조건이 성립할 수 있는 자리는 그 씬뿐이므로 오브젝트도 거기서 동작한다.
     *
     * 조건의 **양쪽**을 다 본다 — `null == SaveLoadController._instance` 처럼 주어가 오른쪽에 오는
     * 비교가 실제로 있다. 짧은 이름이 두 타입에 걸리거나 그 타입이 여러 씬에 놓였으면 버린다.
     * 반쪽짜리 단서로 자리를 정하면 QA agent 가 없는 컨트롤을 찾으러 간다.
     */
    private fun conditionSubjectAnchors(parkedAt: String, types: List<String>): List<SceneAnchor> =
        recordsOf(types).flatMap { record ->
            record.condition.conjunctiveTests().flatMap { test ->
                listOf(test.left, test.right).mapNotNull { subjectAnchor(parkedAt, it) }
            }
        }

    private fun subjectAnchor(parkedAt: String, expression: String): SceneAnchor? {
        ...
        return SceneAnchor(
            ...
            steps = listOf(
                AnchorStep(parkedAt, "condition-reads", expression),
    ...
    private companion object {
         ...
         * 의 `IsStreaming` 같은 멤버 이름이 타입 이름과 우연히 맞아 엉뚱한 씬을 집는다.
         */
...
/**
 * persistent 오브젝트를 어떤 씬으로 옮겼고 **무엇을 읽고 그렇게 판정했나.**
 *
...
/**
 * persistent 오브젝트를 씬에 앉힌 규칙. **선언 순서가 곧 우선순위**이고, [resolution] 이 그 단계의
 * 확실성이다.
 *
 ...
 *
 * `either` 안쪽은 반대쪽 `branch`로 만족될 수 있어 조건을 단정하지 못한다. `ConditionBranches` 가 씬
 ...
 *
 * 부분 일치로 넓히지 않는다 — `GetActiveScene().buildIndex` 같은 다른 조건까지 씬 이름으로 오인한다.
 */
internal const val ACTIVE_SCENE_NAME = "SceneManager.GetActiveScene().name"

/**
 * `"\"GameClearScene\""` 처럼 **따옴표까지 값에 든** 항에서 씬 이름을 꺼낸다.
 *
 * 근거는 IL 의 문자열 리터럴을 소스 표기 그대로 렌더해 넣는다. 따옴표를 벗기지 않고 비교하면 실측
 * 12건이 전부 어긋나 어떤 씬에서도 남지 않는다.
 */
