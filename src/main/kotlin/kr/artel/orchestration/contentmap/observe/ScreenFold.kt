...
/**
 * 한 게임 인스턴스의 판독을 접어 든 것. **화면을 가르는 임계값이 여기 있다.**
 *
 * ## 접기 규칙
 *
 * 전량 판독(`whole=true`)은 교체하고 델타는 얹는다. **말하지 않은 객체는 있던 자리를 지킨다** —
 * 없다는 것이 소식이 아니고, 달라졌다면 판독이 실어 왔을 것이다. agent-server 의
 * `PulseMemory.apply` 와 글자 그대로 같은 규칙이다. 이 채널의 소비자가 둘인데 델타를 다르게
 * 읽으면, 화면이 갈린 이유를 두 쪽에서 다르게 설명하게 되고 어느 쪽이 틀렸는지 가릴 수 없다.
 *
 * 객체를 `씬/selector` 로 키잡는 것도 같은 이유다. 씬이 바뀌면 전량 판독이 오지만, 오지 않는
 * 경로(전달 유실 뒤 델타만 도착)에서 이전 씬의 객체가 남아 판별자를 오염시킨다. 씬을 키에 넣어
 ...
 * | 후보 | 왜 버렸나 |
 * |---|---|
 * | 활성 오브젝트 **전체**의 집합 | 화면이 폭발한다. 적·카드·탄환이 하나 생기고 죽을 때마다 새 화면이 된다. 실측 전투 씬은 손패 장수만으로도 판별자가 매 초 바뀐다 |
 ...
 * | **조작 가능한 객체의 켜짐/꺼짐** (고름) | 화면이 존재하는 이유와 정확히 같은 축이다 — 여기서 무슨 스텝을 실행할 수 있는가. 게임플레이 오브젝트의 생멸도, 값의 변화도 판별자를 흔들지 않는다 |
 *
 * "조작 가능한 객체" 는 두 출처의 합집합이다:
 * - 판독이 `offers` 를 실어 준 객체 — 그 오브젝트가 **지금** 무엇에 응답하는지 SDK 가 말한 것
 * - 그 씬 `capability.control_selector` 가 지목한 객체 — 근거가 컨트롤이라고 말한 것
 *
 * 둘 다 필요하다. `offers` 만 쓰면 옛 SDK(그 칸을 안 보낸다)에서 판별자가 통째로 비고,
 * `control_selector` 만 쓰면 근거가 놓친 팝업이 안 갈린다.
 *
 * ### 임계값이 틀렸을 때 나오는 증상
 *
 ...
 *   그 컨트롤에 기능 행이 없고 SDK 도 `offers` 를 안 보내는 것이다. 그때는 판별자를 활성
 *   오브젝트 집합 쪽으로 넓히되, 넓히는 순간 위쪽 증상이 온다
 *
 * ### 정착(settling)
 *
 * 같은 판별자가 [SETTLE_READINGS] 회 **연속** 관측돼야 화면으로 굳는다. 판독은 초당 여러 번
 * 오고 전이 중간 프레임은 반쯤 지어진 UI 를 보여준다 — 그 한 프레임이 화면 행과 전이 두 개를
 * 만들면 재현 경로에 아무도 본 적 없는 화면이 낀다. 사람이 한 프레임도 못 본 화면은 화면이
 * 아니다.
 *
 * 대가: 정확히 한 판독 동안만 존재하는 화면은 기록되지 않는다. 2 를 고른 것은 그 대가가 가장
...
class ScreenFold {
    ...
    /** 마지막으로 굳은 판별자. 같은 것이 다시 굳어도 그것은 재방문이 아니라 체류다. */
    var settled: ScreenDiscriminator? = null
        private set

    /** 마지막으로 굳은 판별자가 앉은 화면 행. 전이의 출발점이다. */
    ...
    /**
     * 지금 상태의 판별자. [controlSelectors] 는 이 씬 기능들이 지목한 컨트롤이다.
     *
     * selector 로 정렬한다. jsonb 배열은 순서가 있어, 정렬하지 않으면 같은 화면이 판독마다 다른
     * 판별자가 되고 `uk_screen_discriminator` 가 매번 새 행을 앉힌다.
     */
    ...
    /**
     * 이 판별자를 지금 화면으로 굳혀야 하는가.
     *
     * 같은 화면에 머무는 동안에는 계속 `false` 다 — 그래서 `observed_count` 가 판독 수가 아니라
     * **방문 수**를 센다. 재방문이 행을 늘리지 않는 것과 같은 규율이다.
     *
     * **판정만 하고 굳히지는 않는다.** 굳히는 것은 적재가 끝난 뒤 [confirm] 이다. 여기서 굳혀
     * 버리면 적재가 실패했을 때 상태만 앞서 나가고, 같은 판별자가 계속 와도 [settle] 이 늘
     * `false` 를 돌려주어 **그 화면은 영영 앉지 못한다.** 지금 모양에서는 실패 뒤 두 판독이면
     * 다시 시도한다.
     */
    ...
    /** 적재가 끝났다. 이 판별자를 굳히고 다음 전이의 출발점으로 삼는다. */
    ...
    companion object {
        /** 판별자가 화면으로 굳기까지 필요한 연속 관측 수. 근거는 클래스 주석의 `정착` 절. */
