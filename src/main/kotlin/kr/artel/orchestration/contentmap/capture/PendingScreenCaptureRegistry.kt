...
/**
 * 아직 답이 오지 않은 화면 `capture` 청구 하나 (ARTEL-456).
 *
 * @property requestId SDK 로 내보낸 `action` 의 바깥 id. `ACTION_RESULT` 의 `requestId` 가 이것을
 *   되돌려주고, 그 값이 곧 이 청구를 우리 것으로 알아보는 유일한 표다
 * @property screenId 이 그림을 묶을 화면. 이 값이 청구 시점에 정해지는 것이 요점이다 —
...
/**
 * 답을 기다리는 화면 `capture` 청구들 (ARTEL-456).
 *
 ...
 * 자체가 그 프로세스의 것이기 때문이다. 재시작 뒤에도 남는 자리에 두면 아무도 답하지 않을 청구가
 * 영원히 쌓인다. `ScanStatusRegistry` 와 같은 판단이다.
 *
 * ## 런이 끝나거나 socket 이 닫히면
 *
 * **아무 일도 일어나지 않는다.** 청구는 claim 되지 않은 채 [TIME_TO_LIVE] 뒤에 버려지고,
...
class PendingScreenCaptureRegistry(private val clock: Clock) {
    ...
    private companion object {
        ...
        /**
         * 동시에 답을 기다릴 수 있는 청구 수.
         *
         * 새 화면 하나에 청구 하나이므로, 여기 닿는다는 것은 한 서버가 수백 개의 화면을 동시에
