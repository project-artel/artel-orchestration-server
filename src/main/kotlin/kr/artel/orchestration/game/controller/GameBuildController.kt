...
class GameBuildController(
    ...
    ): Mono<GameBuildListResponse> =
        session(jwt)
            .flatMap { buildService.list(it.userId, projectId) }
    ...
    fun update(
        ...
        session(jwt)
            .flatMap { buildService.update(it.userId, projectId, buildId, request) }
            .switchIfEmpty(Mono.error(buildNotFound()))
