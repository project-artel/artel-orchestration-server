...
/**
 * 한 프로젝트의 **앵커가 달린 지식 전부**를 씬 이름과 함께 한 번에 읽는다(ARTEL-611).
 *
 * [KnowledgeAnchorRepository.findVisibleFor][kr.artel.orchestration.knowledge.repository.KnowledgeAnchorRepository]
 * 와 묻는 질문이 다르다. 저쪽은 "이 히트들의 앵커"라 검색이 이미 고른 id 묶음으로 들어오고,
 * 이쪽은 "이 프로젝트에서 앵커가 달린 것 전부"라 입력이 프로젝트 하나다. 씬마다 부르지 않는
 * 것이 이 조회의 존재 이유이므로, 씬 이름을 인자로 받지 않는다.
 *
 * **스코프 술어는 [KnowledgeScopeSql.VISIBLE] 하나를 그대로 지난다.** 손으로 다시 적으면
 * 언젠가 한 곳이 빠지고, 빠진 격리는 조용히 틀린 결과를 낸다 — 가려졌어야 할 지식이 앵커째
...
interface AnchoredKnowledgeRepository : CoroutineCrudRepository<KnowledgeAnchorEntity, Long> {
    ...
    /**
     * [projectId] 에서 [scopeId] 스코프에 보이는, **앵커가 달린** 지식을 씬 이름과 함께 낸다.
     *
     ...
     *
     * 소프트삭제된 지식은 뺀다 — 읽기 경로에서 사라진 지식의 앵커만 남아 나오면 agent 가 이미
     * 지워진 사실을 계속 참으로 읽는다.
     *
