...
interface ProjectDocumentRepository : CoroutineCrudRepository<ProjectDocumentEntity, Long> {

    /** 최신 버전이 앞에 오도록 정렬해 반환한다. */
    fun findByProjectIdOrderByVersionDesc(projectId: Long): Flow<ProjectDocumentEntity>

    suspend fun findFirstByProjectIdOrderByVersionDesc(projectId: Long): ProjectDocumentEntity?

    suspend fun findByIdAndProjectId(id: Long, projectId: Long): ProjectDocumentEntity?

    /** 프로젝트 단위 파일 중복 검증용. 같은 프로젝트에 동일 content_hash가 이미 있는지. */
    suspend fun existsByProjectIdAndContentHash(projectId: Long, contentHash: String): Boolean
    ...
    suspend fun findMaxVersion(projectId: Long): Int
    ...
    fun findLatestByProjectIds(projectIds: Collection<Long>): Flow<ProjectDocumentEntity>
    ...
    fun countByProjectIds(projectIds: Collection<Long>): Flow<ProjectDocumentCount>
    ...
    suspend fun updateParseStatus(documentId: Long, parseStatus: String): Long
}
