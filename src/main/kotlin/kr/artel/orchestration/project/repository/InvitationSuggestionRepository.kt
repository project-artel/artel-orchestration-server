...
class InvitationSuggestionRepository(
     ...
     *
     * 빠지는 사람이 셋이다. 이미 멤버인 사람, 이미 답을 기다리는 초대가 나간 사람, 그리고 확인을
     * 마친 이메일이 없는 사람이다. 셋째가 있는 것은 초대가 결국 이메일로 배달되기 때문이다 —
     * 주소가 없는 사람을 후보로 내면 골랐을 때 보낼 곳이 없다.
     *
    ...
    /** 초대 대상으로 지목된 계정이 실제로 부를 수 있는 사람인지. 아니면 null. */
    suspend fun reachableEmailOf(appUserId: Long): String? =
        databaseClient.sql(
            """
            SELECT email FROM app_user
            WHERE id = :appUserId AND email IS NOT NULL AND email_verified_at IS NOT NULL
            """
        )
            .bind("appUserId", appUserId)
            .map { row, _ -> row.get("email", String::class.java) }
            .one()
            .awaitFirstOrNull()
...
private fun searchSql(match: SuggestionMatch): String {
    ...
    return """
        ...
        WHERE u.email IS NOT NULL
          ...
          AND (lower(u.email) = :query OR $nameCondition)
          AND NOT EXISTS (
