...
class SdkPerformanceRepository(private val db: DatabaseClient) {
    ...
    suspend fun aggregateSample(runId: Long, instanceId: Long, at: Instant, message: SdkPerformanceMessage) {
        ...
        db.sql(SUMMARY_UPSERT).bind("runId",runId).bind("instanceId",instanceId).bind("at",at)
            ...
            .bind("hitches",f.hitchCount).bind("budget",f.budgetMs*f.frameCount)
    ...
    companion object {
        private const val SUMMARY_UPSERT="""INSERT INTO sdk_performance_run_summary(qa_run_id,game_instance_id,game_build_id,is_editor,sample_count,covered_ms,frame_count,frame_time_sum_ms,frame_p95_weighted_sum,frame_p99_weighted_sum,one_low_weighted_sum,hitch_count,budget_weighted_sum,budget_mode_ms,process_sample_count,cpu_weighted_sum,cpu_weight_ms,cpu_percent_max,working_set_bytes_max,gen0_collections,gen1_collections,gen2_collections,discharging_sample_count,updated_at)
          SELECT :runId,:instanceId,gi.last_game_build_id,dc.is_editor,1,:sampled,:frames,:sampled,:p95,:p99,:oneLow,:hitches,:budget,
            (SELECT budget_ms FROM sdk_performance_sample WHERE qa_run_id=:runId GROUP BY budget_ms ORDER BY COUNT(*) DESC,budget_ms LIMIT 1),
        ...
        private const val BUILD_SQL="""SELECT r.id run_id,r.game_instance_id,r.started_at,r.completed_at,r.status,s.* FROM sdk_performance_run_summary s JOIN qa_run r ON r.id=s.qa_run_id JOIN test_run tr ON tr.id=r.test_run_id JOIN project_member pm ON pm.project_id=tr.project_id AND pm.app_user_id=:userId WHERE tr.project_id=:projectId AND s.game_build_id=:buildId AND s.is_editor=FALSE ORDER BY r.started_at"""
