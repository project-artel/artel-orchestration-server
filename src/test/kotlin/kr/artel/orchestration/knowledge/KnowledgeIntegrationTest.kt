...
class KnowledgeIntegrationTest {
    ...
    fun testStoresBatchAndQueriesByProject() {
        ...
        knowledgeService.store(
            ...
            sourceId = 10,
            contentHash = "abc123",
            items = listOf(item("CONTROL", "이동", "WASD로 이동"), item("INFO", "체력", "최대 100"))
        ).block(Duration.ofSeconds(5))

        val all = listByFilters(projectId)
        assertThat(all.items).hasSize(2)
        assertThat(all.items.map { it.tag }).containsExactlyInAnyOrder("CONTROL", "INFO")
    ...
    fun testSkipsInvalidItems() {
        ...
        assertThat(rows[0].tag).isEqualTo("INFO")
    ...
    fun testFiltersBySourceAndTag() {
        ...
        knowledgeService.store(projectId, KnowledgeSource.QA, 20, null, listOf(item("INFO"), item("CONTROL"))).block(Duration.ofSeconds(5))
        ...
        assertThat(listByFilters(projectId, source = "qa", tag = "info").items).hasSize(1)
    }
    ...
    fun testEmptyOrAllInvalidBatchStoresNothing() {
        ...
        // enum 값 존재 확인(회귀 방지)
        assertThat(KnowledgeTag.NAMES).containsExactlyInAnyOrder("CONTROL", "INFO", "MISC")
