...
/**
 * 지식을 씬·화면에 묶는 앵커(ARTEL-591)의 쓰기 경로 통합 테스트.
 *
 * 여기서 보는 것은 세 가지다.
 * 1. `KNOWLEDGE_CREATE`가 실은 앵커가 실제로 저장되고, 지식과 **같은 트랜잭션**에서 만들어진다.
 * 2. 앵커를 싣지 않은 요청은 이 기능 이전과 완전히 같다 — 앵커가 없는 지식이 게임 전체의
 *    사실이고 그것이 기본값이다.
 * 3. 중복 앵커를 DB가 막는다. `screen_id IS NULL` 쪽까지 막는 것이 요점이다 — Postgres는 UNIQUE에서
 *    NULL을 서로 다른 값으로 보므로 부분 유니크 인덱스가 없으면 그쪽만 무한히 쌓인다(V55).
 *
 * 읽기 쪽(검색 응답에 앵커가 실리는지, `scene_name` 필터, 스코프 가시성)은
...
class KnowledgeAnchorIntegrationTest {
    ...
    @Test
    fun `앵커를 실은 생성은 씬과 화면을 함께 남긴다`(): Unit = runBlocking {
     ...
     * 그때 앵커는 씬까지만 말하고 멈춰야지, 요청이 거절되어서는 안 된다.
     */
    @Test
     ...
     *
     * 두 번째 앵커부터는 아직 쓰기 경로가 없다(앵커 수정 API는 ARTEL-591의 non-goal). 표가 그것을
    ...
    /**
     * 회귀 방어. 앵커를 싣지 않은 프레임은 이 기능 이전과 **완전히 같아야 한다** — 앵커가 없는
     * 지식이 게임 전체의 사실이고, 그것이 기본값이라 싸야 한다.
     */
    @Test
    fun `앵커를 싣지 않은 생성은 지금까지와 같다`(): Unit = runBlocking {
        val projectId = projectSeq.incrementAndGet()
    ...
    /**
     * 화면은 씬 안에 산다(V55). 씬을 모르는 화면 앵커를 조용히 저장하면 나중에 어느 씬의 화면이었는지
     * 되짚을 수 없어 그 앵커가 영영 반쪽으로 남는다. **지식 자체도 저장되지 않아야 한다** — 앵커를
    ...
    /**
     * **이 테스트가 V55의 부분 유니크 인덱스 두 벌이 있는 이유다.** 중복 앵커는 조용히 틀린다 —
     ...
     * content map이 채워지기 전까지 **모든** 앵커가 이쪽이라 여기가 실제로 일하는 쪽이다.
     */
    @Test
    fun `화면을 모르는 앵커도 씬마다 하나뿐이다`(): Unit = runBlocking {
        val projectId = projectSeq.incrementAndGet()
    ...
    /** 같은 씬의 다른 화면은 별개의 앵커다. 유일성이 지나치게 넓으면 그것도 잘못이다. */
    @Test
    fun `같은 씬의 다른 화면은 막지 않는다`(): Unit = runBlocking {
        ...
        anchorRepository.save(KnowledgeAnchorEntity(knowledgeId = knowledgeId, sceneName = "Combat", screenId = 2L))
        // 화면을 모르는 앵커도 화면을 아는 앵커와 공존한다 — 서로 다른 사실이다.
    ...
    /**
     * 앵커에는 자기 스코프가 없다 — knowledge 행이 스코프를 지고, 조회가 그 행을 조인해
     * `KnowledgeScopeSql.VISIBLE`을 지난다(V55). 그 술어가 빠지면 스코프에 가려졌어야 할 지식의
     * 앵커가 새는데, 앵커만 보면 그것이 어느 지식의 것인지 알 수 없어 새는 것을 알아채기도 어렵다.
     */
    @Test
    fun `가려진 지식의 앵커는 조회되지 않는다`(): Unit = runBlocking {
        val projectId = projectSeq.incrementAndGet()
        ...
        // 이 그림자가 baseline을 가린다 — baseline의 앵커도 함께 가려져야 한다.
