...
/**
 * CSV 읽기와 XLSX 변환 단위 테스트. Spring 컨텍스트도 DB도 필요 없다.
 *
...
class CsvToXlsxConverterTest {
    ...
    @Test
    fun `변환한 XLSX를 다시 읽으면 CSV 값이 그대로다`(): Unit = runBlocking {
        val csv = """
    ...
    @Test
    fun `숫자로 보이는 값도 문자열로 남아 앞자리 0이 살아있다`(): Unit = runBlocking {
        val csv = """
            code,buildId,title
    ...
    @Test
    fun `따옴표로 감싼 콤마와 개행이 한 칸으로 유지된다`(): Unit = runBlocking {
    ...
    @Test
    fun `헤더만 있는 CSV는 헤더 한 줄짜리 시트가 된다`(): Unit = runBlocking {
    ...
    @Test
    fun `빈 CSV는 400으로 거절한다`(): Unit = runBlocking {
        assertThatThrownBy { runBlocking { reader.read(ByteArray(0)) } }
    ...
    @Test
    fun `행 수 상한을 넘으면 거절한다`(): Unit = runBlocking {
        val csv = buildString {
            appendLine("title,expected")
            repeat(6) { appendLine("케이스 $it,기대 $it") }
        }.toByteArray()

        assertThatThrownBy { runBlocking { reader.read(csv, maxRows = 5) } }
    ...
    @Test
    fun `BOM이 붙어도 첫 헤더 이름을 알아본다`(): Unit = runBlocking {
    ...
    @Test
    fun `헤더 표기가 달라도 같은 열로 본다`(): Unit = runBlocking {
    ...
    @Test
    fun `끝에 남은 빈 줄은 행으로 세지 않는다`(): Unit = runBlocking {
    ...
    private suspend fun convertAndRead(csv: ByteArray): List<List<String>> {
        val bytes = converter.convert(reader.read(csv))
