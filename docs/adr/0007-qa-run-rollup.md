...
- `qa_try` 하나가 끝날 때마다 그 부모 `qa_run` 의 자식을 전부 다시 읽고, 전부 끝났으면 `qa_run` 을
...
- 종료 전이가 아예 빠져 있었습니다. 취소든 실패든 정상 완료든 **`qa_try` 만 끝나고 부모 `qa_run` 은
...
| 종료 사유를 message 문구로만 구분한다 | 거절 셋이 다 409 라 클라이언트가 그 문구로 갈랐고, "시나리오가 없는 테스트 런" 이 "이미 진행 중인 QA" 로 표시됨 |
