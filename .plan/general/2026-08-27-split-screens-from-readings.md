# 2026-08-27 — 판독에서 화면을 가르고 화면 전이를 남긴다
...
QA 런이 흘리는 `PULSE` 판독만으로 `screen` · `screen_capability` ·
...
- 액션-판독 시간축 부착(ARTEL-450). **선행이 안 끝났다** — 그래서
  `screen_transition.capability_id` 는 이번에 전부 null 이고 `fired_count` 는 늘 0 이다.
- 읽기 API · 뷰 DTO. ARTEL-596 이 가져간다.

## Context / Constraints

판독 문서의 모양(agent-server `app/qa/pulse.py` 와 SDK `LiveState.cs` 가 같은 것을 읽는다):
...
- [x] **Step 0: Recon** — V40 의 `screen` · `screen_capability` · `screen_transition` ·
      ...
      `ScreenRepositories.kt`, agent-server `pulse.py` 의 접기 규칙.

- [x] **Step 1: 임계값** — 무엇을 다른 화면으로 볼지.
      - 규칙: **조작 가능한 객체의 켜짐/꺼짐만** 판별자에 담는다. 값 변화도, 일반 오브젝트의
        생멸도 담지 않는다.
      - 조작 가능한 객체 = 판독이 `offers` 를 실어 준 객체 ∪ 그 씬 `capability.control_selector`
        가 지목하는 객체.
      - 정착(settling): 같은 판별자가 **연속 2회** 관측돼야 화면이 된다. 전이 중간 프레임이
        유령 화면을 만들지 않게.
      - 안전판: 한 씬당 화면 **32개**를 넘으면 더 만들지 않고 WARN 을 남긴다.

- [x] **Step 2: 구현**
      - `contentmap/observe/PulseReading.kt` — 판독 문서를 경계에서 한 번 타입으로 판다.
      - `contentmap/observe/ScreenFold.kt` — 인스턴스별 접기 + 판별자 산출 + 정착.
...
- [x] **Step 3: 테스트** — `ScreenObservationTest`
      ...
      - **임계값 고정** — 감시 멤버 값만 바뀐 판독은 화면을 가르지 않는다

- [x] **Step 4: Rollout** — 스키마 추가 하나뿐이고 기존 행을 건드리지 않는다.
      QA 런 없이 온 판독은 무시하므로 스캔 순회는 화면을 만들지 않는다.
