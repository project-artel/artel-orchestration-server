# 2026-08-27 — 지식 그래프 조회에 `anchor` 를 싣는다
...
ARTEL-591 이 만든 `anchor` 를 **사람이 볼 수 있게** 한다. 591 은 `anchor` 를 저장하고 Agent 의
...
`anchor` 가 DB 에 있어도 화면에는 없는 것과 같고, ARTEL-593(FE) 이 그릴 것이 없다.

- 그래프 노드마다 `anchors` 를 싣는다. **없으면 빈 배열**이고 그 필드는 언제나 있다.
- 노드 묶음의 `anchor` 를 **한 번에** 데려온다. 노드당 한 질의는 응답 하나를 N 질의로 만든다.
- 스코프에 가려진 지식의 `anchor` 는 새지 않는다.

## Non-goals

- `anchor` 수정·삭제 API (ARTEL-591 과 같은 non-goal).
- `anchor` 로 그래프를 좁히는 필터. 검색에는 `scene_name` 필터가 있지만(591) 그래프는 창고
...
- **`anchor` 가 없는 것이 기본값이다.** `anchor` 없는 지식은 게임 전체의 사실이고, 창고 대부분이
  그렇다. 그래서 빈 배열은 "못 읽었다"가 아니라 "게임 전체의 사실"이라는 뜻이다.
- **N+1 을 만들지 않는다.** 이 조회는 노드를 최대 500개까지 낸다. 노드마다 `anchor` 를 부르면
  질의가 노드 수만큼 늘고, 그 비용은 `anchor` 가 하나도 없는 프로젝트에서도 그대로 난다.
  `KnowledgeAnchorRepository.findVisibleFor` 가 이미 묶음 조회라 그대로 쓴다.
- **스코프 술어는 리포지토리가 진다.** 그래프는 운영 스코프만 담으므로 노드는 이미
  걸러진 뒤지만, `anchor` 조회가 자기 힘으로 `KnowledgeScopeSql.VISIBLE` 을 지나야 한다 —
...
- **Risks:**
  - 노드 상한이 500 이라 `anchor` 행이 노드보다 많을 수 있다(한 지식이 여러 화면에 걸린다).
    응답 크기가 `anchor` 수만큼 는다 — `anchor` 는 씬 이름과 id 한 쌍뿐이라 본문보다 훨씬 작고,
    노드가 본문을 싣지 않는 기존 판단이 이 여유를 이미 벌어 두었다.
  - 591 이 남긴 구멍(스코프 런의 그림자가 baseline `anchor` 를 물려받지 않는다)은 그대로다.
...
- `anchor` 로 그래프를 좁히는 축이 필요해지면 검색의 `scene_name` 필터와 같은 모양으로
  붙인다. 이번 범위 밖이라 열어 둔다.
