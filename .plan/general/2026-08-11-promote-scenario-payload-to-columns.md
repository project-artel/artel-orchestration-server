...
- Date: 2026-08-11
- Jira: ARTEL-291
- Status: Implemented — PR #101 (https://github.com/project-artel/artel-orchestration-server/pull/101)
...
- [x] **Step 0: Recon** — 완료. 소비자 7곳·테스트 4개 식별(위 표).

- [x] **Step 1: 마이그레이션** `src/main/resources/db/migration/V32__promote_test_scenario_payload_columns.sql`
...
- [x] **Step 2: 엔티티** `TestScenarioEntity`
...
- [x] **Step 6: diff 리뷰 + 커밋** — 스코프 밖 변경 없는지 전수 확인
...
- 계획에 없던 정리 하나: `ScenarioCompositionService`가 payload 파싱을 잃으면서 `ObjectMapper` 생성자 의존이 죽어 함께 제거했다. 죽은 인자 `testScenarioId`·`userId` 제거는 계획대로.
- `TestScenarioPipelineIntegrationTest`의 승인 테스트에 재설계 이전 스텝 리터럴이 남아 있어 현행 계약 형태로 고쳤다(그대로 두면 단언이 무의미).
- 검증 결과: `./mvnw test` 405/405, flyway 검사 OK, 백필 수동 검증 3행 통과.
