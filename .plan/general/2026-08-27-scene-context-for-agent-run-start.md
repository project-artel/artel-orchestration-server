# 2026-08-27 — 씬별 capability 와 `anchor` 지식을 한 번에 내주는 내부 창구
...
- `anchor` 없는 지식. 그것은 게임 전체의 사실이고 지식창고의 대부분이라, 검색으로 찾는 것으로
  남긴다.
...
`anchor` 는 content map 과 **대조하지 않고** 저장된다(ARTEL-591, V55 — content map 이 없는
프로젝트도 씬 이름은 있기 때문이다). 그래서 content map 이 들어 본 적 없는 씬 이름을 든
`anchor` 가 정상적으로 존재한다. 그런 씬도 응답에 담고, capability 는 빈 목록으로 낸다. 빼면
...
`anchor` 지식은 매 모델 호출마다 다시 그려지는 프롬프트 블록으로 간다. 본문을 실으면 그
...
`kr.artel.orchestration.scenecontext` 안에서만 쓴다. `anchor` 조회도 새 리포지토리를 이 패키지에
...
- [ ] **Step 1: DTO** — `scenecontext/dto/SceneContextDtos.kt`. 응답 루트 · 씬 · capability ·
      `anchor` 지식. id 계열은 문자열(64비트 정밀도).
- [ ] **Step 2: `anchor` 조회** — `scenecontext/repository/AnchoredKnowledgeRepository.kt`.
...
1. capability 와 `anchor` 가 다 있는 빌드 — 둘이 같은 씬 키 아래 붙는다
2. `anchor` 에만 있는 씬 — 응답에 있고 capability 가 빈 목록이다
3. content map 이 없는 빌드 — 404 가 아니라 200, 씬 목록은 비고 `anchor` 씬만 나온다
4. `not-a-step` 이 나가지 않는다
5. `description` 이 payload 어디에도 없다
6. 스코프에 가려진 지식이 `anchor` 째 빠진다
